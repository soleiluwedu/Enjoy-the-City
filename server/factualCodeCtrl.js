...
// Factual.com Places API codes corresponding to categories of venues.
const factualCodes = {
	landmark: 107,
	garden: 109,
	natural: 112,
...
// All accepted routes and the associated arrays of codes and descriptions used to send data to client.
...
// Saves appropriate codes and descriptions to res.locals.codesToReq.
const factualCodeCtrl = {

	// Checks req.url and if valid, and saves corresponding
	// array of codes and descriptions in res.locals.codesToReq.
	setCodes: (req, res, next) => {
		...
		// Search for the req.url (minus the /) as a key.
		const route = req.url.slice(1);

		// If route is valid, save array of codes and descriptions in res.locals.codesToReq.
		...
		// If route is not valid...
			...
			// Save error object to res.locals.err to be passed to serverside logger (last middleware).
			res.locals.err = new Error(`Error 404 sent to client. Route '${req.url}' not found.`);
		}

		// Move on to next middleware regardless of success or error, to log report serverside.
		return next();
	}
}

module.exports = factualCodeCtrl;
